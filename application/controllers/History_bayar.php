...
class History_bayar extends CI_Controller {
	...
	public function index()
	{
		...
		if(isset($_POST['cari_data'])){
			...
			$data_bayar = $this->M_master->tampil_history_bayar_tanggal($tanggal_from, $tanggal_to)->result_array();

		}elseif(isset($_POST['cari_data2'])){
			$sub_biaya = $this->input->post('sub_biaya');
			$data_bayar = $this->M_master->tampil_history_bayar_biaya($sub_biaya)->result_array();

		}elseif(isset($_POST['cari_data3'])){
			$cabang = $this->input->post('cabang');
			$data_bayar = $this->M_master->tampil_history_bayar_cabang($cabang)->result_array();

		}elseif(isset($_POST['cari_data4'])){
			$departemen = $this->input->post('departemen');
			$data_bayar = $this->M_master->tampil_history_bayar_departemen($departemen)->result_array();

		}elseif(isset($_POST['reset_filter'])){
			$data_bayar = $this->M_master->tampil_history_bayar()->result_array();
		...
		$this->load->view('v_history_bayar', array(
	...
	public function detail($id){
		...
		$this->load->view('header');
		$this->load->view('sidebar', array('data_jb'=>$data_jb));
		$this->load->view('v_p_detail_history_bayar', array(
