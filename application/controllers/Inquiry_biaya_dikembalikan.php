...
class Inquiry_biaya_dikembalikan extends CI_Controller {
	...
	public function index()
	{
		...
		if($cabang=='HEAD OFFICE'){ // Jika Kantor Pusat
			...
			if(isset($_POST['cari_data1'])){
				...
				$data_inquiry = $this->db->query("SELECT * FROM tbl_penyelesaian_dikembalikan INNER JOIN tbl_pengajuan USING(nomor_pengajuan) WHERE tbl_pengajuan.bagian='$departemen' AND tbl_pengajuan.sub_biaya='$sub_biaya' ORDER BY tbl_penyelesaian_dikembalikan.id_penyelesaian DESC")->result_array();

			}elseif(isset($_POST['cari_data2'])){

				$status_penyelesaian = $this->input->post('status_penyelesaian');
				$data_inquiry = $this->db->query("SELECT * FROM tbl_penyelesaian_dikembalikan INNER JOIN tbl_pengajuan USING(nomor_pengajuan) WHERE tbl_pengajuan.bagian='$departemen' AND tbl_penyelesaian_dikembalikan.status_verifikasi_penyelesaian='$status_penyelesaian' ORDER BY tbl_penyelesaian_dikembalikan.id_penyelesaian DESC")->result_array();

			}else{
				$data_inquiry = $this->db->query("SELECT * FROM tbl_penyelesaian_dikembalikan INNER JOIN tbl_pengajuan USING(nomor_pengajuan) WHERE tbl_pengajuan.bagian='$departemen' ORDER BY tbl_penyelesaian_dikembalikan.id_penyelesaian DESC")->result_array();
			}
			...
			if($level == 'Branch Manager'){
				$data_inquiry = $this->db->query("SELECT * FROM tbl_penyelesaian_dikembalikan INNER JOIN tbl_pengajuan USING(nomor_pengajuan) WHERE tbl_pengajuan.cabang='$cabang' ORDER BY tbl_penyelesaian_dikembalikan.id_penyelesaian DESC")->result_array();

			}elseif($level == 'Area Manager'){
				$data_inquiry = $this->db->query("SELECT * FROM tbl_penyelesaian_dikembalikan INNER JOIN tbl_pengajuan USING(nomor_pengajuan) WHERE tbl_pengajuan.wilayah='$cabang' ORDER BY tbl_penyelesaian_dikembalikan.id_penyelesaian DESC")->result_array();

			}else{
				if(isset($_POST['cari_data1'])){
				
					$sub_biaya = $this->input->post('sub_biaya');
					$data_inquiry = $this->db->query("SELECT * FROM tbl_penyelesaian_dikembalikan INNER JOIN tbl_pengajuan USING(nomor_pengajuan) WHERE tbl_pengajuan.cabang='$cabang' AND tbl_pengajuan.bagian='$level' AND tbl_pengajuan.sub_biaya='$sub_biaya' ORDER BY tbl_penyelesaian_dikembalikan.id_penyelesaian DESC")->result_array();
	
				}elseif(isset($_POST['cari_data2'])){
	
					$status_penyelesaian = $this->input->post('status_penyelesaian');
					$data_inquiry = $this->db->query("SELECT * FROM tbl_penyelesaian_dikembalikan INNER JOIN tbl_pengajuan USING(nomor_pengajuan) WHERE tbl_pengajuan.cabang='$cabang' AND tbl_pengajuan.bagian='$level' AND tbl_penyelesaian_dikembalikan.status_verifikasi_penyelesaian='$status_penyelesaian' ORDER BY tbl_penyelesaian_dikembalikan.id_penyelesaian DESC")->result_array();
	
				}else{
					$data_inquiry = $this->db->query("SELECT * FROM tbl_penyelesaian_dikembalikan INNER JOIN tbl_pengajuan USING(nomor_pengajuan) WHERE tbl_pengajuan.cabang='$cabang' AND tbl_pengajuan.bagian='$level' ORDER BY tbl_penyelesaian_dikembalikan.id_penyelesaian DESC")->result_array();
				}
			}
		...
		$this->load->view('v_inquiry_dikembalikan', array('data_inquiry' => $data_inquiry));
	...
	public function detail($id){
		...
		$this->load->view('header');
		$this->load->view('sidebar', array('data_jb'=>$data_jb));
		$this->load->view('v_inquiry_dikembalikan_detail', array(
