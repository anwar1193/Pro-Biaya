	...
	public function index()
	{
		...
		if($cabang=='HEAD OFFICE'){ // Jika Kantor Pusat
			...
			if($level == 'Branch Manager'){
				$data_inquiry = $this->db->query("SELECT * FROM tbl_penyelesaian_kelebihan INNER JOIN tbl_pengajuan USING(nomor_pengajuan) WHERE tbl_pengajuan.cabang='$cabang' ORDER BY tbl_penyelesaian_kelebihan.id_penyelesaian DESC")->result_array();

			}elseif($level == 'Area Manager'){
				$data_inquiry = $this->db->query("SELECT * FROM tbl_penyelesaian_kelebihan INNER JOIN tbl_pengajuan USING(nomor_pengajuan) WHERE tbl_pengajuan.wilayah='$cabang' ORDER BY tbl_penyelesaian_kelebihan.id_penyelesaian DESC")->result_array();
