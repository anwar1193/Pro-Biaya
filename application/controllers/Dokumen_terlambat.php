...
class Dokumen_terlambat extends CI_Controller {
	...
	public function index()
	{
		...
		$data_dokterlambat = $this->db->query("SELECT * FROM tbl_pengajuan WHERE status_bayar='Telah Dibayar' AND status_dokumen='' AND cabang='$cabang' AND bagian='$identitas' AND datediff(tanggal_bayar, current_date()) < -14 ")->result_array();

		$this->load->view('header');
		$this->load->view('sidebar', array('data_jb'=>$data_jb));
		$this->load->view('v_dokumen_terlambat', array(
	...
	public function detail($id){
		...
		$this->load->view('header');
		$this->load->view('sidebar', array('data_jb'=>$data_jb));
		$this->load->view('v_dok_terlambat_detail', array(
