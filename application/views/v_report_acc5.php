...
<div>
	<span style="margin-left: 20px; font-size: 20px"><b>LAPORAN RINCIAN BMHD</b></span><br>
</div>
...
<table border="1">
	...
	<tbody>
		<?php  
			...
			$data = $this->db->query("SELECT * FROM tbl_pengajuan  WHERE status_bayar='Proses Bayar' AND tanggal<='$tanggal' ORDER BY tanggal_proses_bayar DESC")->result_array();
		...
		<tr>
			...
			<td><?php echo $dt_coa['coa'].'<br>'.$dt_coa['nama_coa'] ?></td>
		...
		<?php  
			// Ambil Totalan
			$data_total = $this->db->query("SELECT SUM(jumlah) AS jumlah_total, SUM(ppn) AS total_ppn, SUM(pph23) AS total_pph23, SUM(pph42) AS total_pph42, SUM(pph21) AS total_pph21, SUM(total) AS grand_total FROM tbl_pengajuan WHERE tanggal<='$tanggal' AND status_bayar='Proses Bayar'")->row_array();
		?>
		<tr style="background-color: silver">
