<?php  
	...
	header("Content-Disposition: attachment; filename=report_transaksi_bmhd.xls");

	date_default_timezone_set("Asia/Jakarta");

	$coa = $data_coa_satu['coa'];
	$nama_coa = $data_coa_satu['nama_coa'];
	// $sub_biaya = $data_coa['sub_biaya'];
...
<table border="1">
	...
	<tbody id="table_data">
		<?php  
			...
			$data = $this->db->query("SELECT * FROM tbl_pengajuan WHERE coa='$coa' AND (tanggal_proses_bayar BETWEEN '$tanggal_from' AND '$tanggal_to') ORDER BY tanggal_proses_bayar DESC")->result_array();
		...
		<tr>
			...
			<td id="jumlah" style="text-align: right;"><?php echo $row['jumlah'] ?></td>
		...
		<?php  
			// Ambil Totalan
			$data_total = $this->db->query("SELECT SUM(jumlah) AS jumlah_total, SUM(ppn) AS total_ppn, SUM(pph23) AS total_pph23, SUM(pph42) AS total_pph42, SUM(pph21) AS total_pph21, SUM(total) AS grand_total FROM tbl_pengajuan WHERE coa='$coa' AND (tanggal_proses_bayar BETWEEN '$tanggal_from' AND '$tanggal_to')")->row_array();
		?>
		<tr style="background-color: silver">
